ActiveAdmin.register Bastion do
  ...
  filter :category, as: :select, collection: Bastion.categories
  filter :level
  filter :description
  filter :published_at
  filter :created_at
  ...
  show do
    ...
    div do
      if resource.published?
        link_to "Unpublish", unpublish_admin_bastion_path(resource), class: "btn btn-primary"
      else
        link_to "Publish", publish_admin_bastion_path(resource), class: "btn btn-primary"
      end
    end
  end
  ...
  form do |f|
    ...
    f.inputs do
      ...
      f.input :level, hint: "Только для специализированных (категория «leveling»)"
      f.input :description,
        label: "Description (#{Bastion::DESCRIPTION_FORMAT})",
        as: :simplemde_editor,
        input_html: {rows: 12, style: "height:auto"}
      f.input :original_description,
        label: "Original description (#{Bastion::DESCRIPTION_FORMAT})",
        as: :simplemde_editor,
        input_html: {rows: 12, style: "height:auto"}
    ...
    unless f.object.new_record?
      f.actions do
        if f.object.published?
          li class: "action" do
            link_to "Unpublish", unpublish_admin_bastion_path(f.object)
          end
        else
          li class: "action" do
            link_to "Publish", publish_admin_bastion_path(f.object)
          end
        end
      end
    end
  ...
  controller do
    def create
      @resource = Bastion.new

      if @resource.update(create_params)
        if params[:create_another] == "on"
          redirect_to new_resource_path(@resource), notice: "Bastion was successfully created. Create another one."
        else
          redirect_to resource_path(@resource), notice: "Bastion was successfully created."
        end
      else
    ...
    def update
      if resource.update(update_params)
        redirect_to resource_path(resource), notice: "Bastion was successfully updated."
      else
        flash.now[:alert] = "Errors happened: " + resource.errors.full_messages.to_sentence
    ...
    def destroy
      if resource.destroy
        redirect_to collection_path, notice: "The bastion has been deleted."
      else
    ...
    def create_params
      attrs = permitted_params[:bastion].to_h
