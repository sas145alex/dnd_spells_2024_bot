ActiveAdmin.register Metamagic do
  ...
  show do
    ...
    div do
      if resource.published?
        link_to "Unpublish", unpublish_admin_metamagic_path(resource), class: "btn btn-primary"
      else
        link_to "Publish", publish_admin_metamagic_path(resource), class: "btn btn-primary"
      end
    end
  end
  ...
  form do |f|
    f.semantic_errors
    f.inputs do
      f.input :sorcery_points
      f.input :title
      f.input :original_title
      f.input :description,
        label: "Description (#{Metamagic::DESCRIPTION_FORMAT})",
        as: :simplemde_editor,
    ...
    unless f.object.new_record?
      f.actions do
        if f.object.published?
          li class: "action" do
            link_to "Unpublish", unpublish_admin_metamagic_path(f.object)
          end
        else
          li class: "action" do
            link_to "Publish", publish_admin_metamagic_path(f.object)
          end
        end
      end
    end
  ...
  controller do
    def create
      @resource = Metamagic.new

      if @resource.update(create_params)
        if params[:create_another] == "on"
          redirect_to new_resource_path(@resource), notice: "Metamagic was successfully created. Create another one."
        else
          redirect_to resource_path(@resource), notice: "Metamagic was successfully created."
        end
      else
    ...
    def update
      if resource.update(update_params)
        redirect_to resource_path(resource), notice: "Metamagic was successfully updated."
      else
        flash.now[:alert] = "Errors happened: " + resource.errors.full_messages.to_sentence
    ...
    def create_params
      attrs = permitted_params[:metamagic].to_h
